...
| `item` | self | `name`, weapon (`element`+`power`)?, `tags` | **Equip** one of your units (P3.2). A *weapon*-item (re)arms it — a flaming sword makes its attacks fire; an *armor/trinket* item adds `tags` (kryptonite armor) that P3.3 matchups read. Applies immediately; stays until the unit dies. Equipping fans out into the unit's `weapon`/`tags` (which drive combat) **and** records a structured `Item{name, kind: weapon\|gear, element?, power?, tags}` on `unit.items` — `kind` inferred server-side (weapon = has power) — for the inspector to render each piece of gear. |
