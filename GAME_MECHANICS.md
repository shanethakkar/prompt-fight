...
- **Match mode (P1):** every match is `sandbox` or `competitive` (a server-tracked `GameState.mode`, chosen at creation, default sandbox). Sandbox ignores the mana/cooldown gate and disables the reliability roll (everything lands); competitive turns both on. The reliability system itself lands in the P1.1/P1.2 stages.
...
| `item` | self | `name`, weapon (`element`+`power`)?, `tags` | **Equip** one of your units (P3.2). A *weapon*-item (re)arms it — a flaming sword makes its attacks fire; an *armor/trinket* item adds `tags` (kryptonite armor) that P3.3 matchups read. Applies immediately; stays until the unit dies. |
