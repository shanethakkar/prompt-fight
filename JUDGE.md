...
- **`summon`** (self, P3.1b) — bring a new entity onto your side. `name`, `hp` **strictly by size** (8–75; a common animal is *not* a tank: rat/imp ~8, dog/goblin ~15, wolf/soldier ~25, orc/knight ~35, ogre/troll/golem ~50, dragon/giant/god ~70), weapon (`element`+`power`) inferred from what it IS (archer→bow, dragon→fire/high-hp), optional `tags`/`item`. To spawn it **already wearing functional armor**, set `armor` (material rating, same scale as an armor item) + `item` = the armor's name (charges a little extra mana). A summon takes the whole command (no attacking); the unit acts next turn. Reference existing units by id (`source_id`/`target_id`) — "my orc attacks their wizard".
