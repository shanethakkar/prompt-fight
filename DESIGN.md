...
> **Landed 2026-07-03 — "Effect grammar (Stage A)"** brought P2's core forward first (it was the real bottleneck): bundled 1–3 typed components (`damage`/`heal`/`dot`/`hot`/`stat`/`defense`), the generic over-time + stat modifier grammar, the component cost model, and persistent stacking effects — all on the alternating-turn engine.
>
> **Landed 2026-07-03 — "Batch B"** followed with: a **cost retune** (per-component exponent so bundles are affordable), **durability armor** (a `barrier` absorb pool that shatters, replacing timed armor), a frontend **combat ledger**, and **real stun** (`control`, formerly A.2 — a turn-skip with anti-lock immunity). Still deferred: **resource** drain (A.3) and everything reliability/mode-related below (P1). The **entity/minion layer stays P3**. See GAME_MECHANICS §3–§8.
>
> **Landed 2026-07-03 — "P3" then "P1".** P3 (entities/summon/items/effectiveness) shipped first (the user's priority). Then **P1 — modes + reliability**: server-tracked `mode` (sandbox/competitive) + a seeded reliability roll — `fizzle → partial → full → overload` + backfire — reframed by the user as **Aptitude × Ambition**: reliability reflects *whether the actor is fit for the action* (server-grounded `fit`/`improvised`/`unfit` — a mage casts, a stickman punches; a wand earns the magic) throttled by how much it bites off (reach), with informed odds shown before Confirm. §8's open questions are now settled in `balance.json → reliability` (see GAME_MECHANICS §7c). Still deferred from P1: blind/accuracy hit-debuffs + charge-to-stabilize.

- **P1 — Modes + reliability on the current engine.** ✅ *Landed (as Aptitude × Ambition — see the callout above).* Add a `mode` config and the reliability spectrum (seeded RNG + informed odds in the cost preview). High-power / ill-suited actions can fizzle / partial / crit / backfire.
