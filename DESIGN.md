...
> **Landed 2026-07-03 — "Effect grammar (Stage A)"** brought P2's core forward first (it was the real bottleneck): bundled 1–3 typed components (`damage`/`heal`/`dot`/`hot`/`stat`/`defense`), the generic over-time + stat modifier grammar, the component cost model, and persistent stacking effects — all on the alternating-turn engine. Deferred out of Stage A: **stun** (A.2), **resource** drain (A.3), and everything reliability/mode-related below (still P1). The **entity/minion layer stays P3**. See GAME_MECHANICS §3–§8.
